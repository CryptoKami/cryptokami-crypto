     ...
     encrypted_key *encrypted_key)
{
	...
	cryptokami_crypto_ed25519_publickey(secret_key, pub_key);
     ...
     encrypted_key *encrypted_key)
{
	...
	if (cryptokami_crypto_ed25519_extend(seed, secret_key))
		return 1;
     ...
     ed25519_signature signature)
{
	...
	cryptokami_crypto_ed25519_publickey(priv_key, pub_key);
	cryptokami_crypto_ed25519_sign(data, data_len, encrypted_key->cc, CHAIN_CODE_SIZE, priv_key, pub_key, signature);
     ...
     encrypted_key *out)
{
	...
	/* Kl = 8*Zl + parent(K)l */
	cryptokami_crypto_ed25519_scalar_add(zl8, priv_key, res_key);
	/* Kr = Zr + parent(K)r */
     ...
     uint8_t *cc_out)
{
	...
	/* Kl = 8*Zl*B + Al */
	cryptokami_crypto_ed25519_publickey(zl8, pub_zl8);
	cryptokami_crypto_ed25519_point_add(pub_zl8, pub_in, pub_out);
